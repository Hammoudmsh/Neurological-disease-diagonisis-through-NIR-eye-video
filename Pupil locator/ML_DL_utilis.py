...
class MLDL_utilitis:
    def __init__(self, where2save = None):
        ...
        self.setTitle(ts = 30, tc = "black", ls = 18)
    ...
    def plotHistory(self, history, n = [1, 2], size = (5,5), show = False, prefix = ""):
        ...
        for metric in what:
            try:
                ...
                if what =="loss":
                    plt.semilogy(history.epoch, history.history[metric], color=colors[n[0]], label='Train Loss' + label)
                    plt.semilogy(history.epoch, history.history['val_'+metric], color=colors[n[1]], label='Val Loss' + label, linestyle="--")
                else:
                    plt.plot(history.epoch, history.history[metric], color=colors[n[0]], label='Train '+metric)
                    plt.plot(history.epoch, history.history['val_'+metric], color=colors[n[1]], label='Val '+metric, linestyle="--")
                if what =="learning rate":
                    plt.plot(history.epoch, metric, color=colors[n[0]], label= metric)
                plt.title(metric, fontsize = self.titleSize)
                plt.xlabel('Epoch')
                plt.ylabel(metric)
                ...
                x = self.where2save
                plt.xlabel('Epoch', fontsize=self.labelSize)
                plt.ylabel(f'{metric}', fontsize=self.labelSize)
...
#         ax.set_xticks(rotation=45)
    ...
    def make_confusion_matrix(self,
                              ...
                              count=True,
                              show = True ,
                              percent=True,cbar=True, xyticks=True, xyplotlabels=True, sum_stats=True, figsize=None, cmap='Blues',title=None, prefix = ""):
        '''
        ...
        if sum_stats:
            ...
            if len(cf)==2:
                ...
                resss = precision_recall_fscore_support(y, y_pred, average=None, labels = range(len(categories)))
                print(resss)
        else:
            stats_text = ""
        ...
        sns.heatmap(cf,annot=box_labels,fmt="",cmap=cmap,cbar=cbar,xticklabels=categories,yticklabels=categories, linewidths=3, linecolor='white')
